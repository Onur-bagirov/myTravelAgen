...
export default function CreateTrainTicket({ onCreated }) {
  ...
  // ── API-dən Location və Variant siyahısını çəkirik ──
  useEffect(() => {
    // Şəhərləri çək
    ...
    // Qiymət variantlarını (Sinifləri) çək
    fetch(`${BASE_URL}/Variant?Page=1&Limit=100`, { headers: authHeaders() })
      .then(r => r.json())
      .then(d => {
        const list = Array.isArray(d?.data) ? d.data : [];
        setVariants(list);
        // İlk variantı default olaraq seçilmiş et
        if (list.length > 0) {
  ...
  const totalSeats = seatGroups.reduce((s, g) => s + g.rowCount * g.seatsPerRow, 0);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setSuccessMsg(`✅ Uğurla yaradıldı!`);
    ...
    <div className="ct-page">
      <div className="ct-wrapper">
        ...
        <form className="ct-form" onSubmit={handleSubmit}>
          ...
          <section className="ct-section">
            ...
            <div className="ct-grid-2">
              <div className="ct-field">
                <label>Şirkət</label>
                <input name="trainCompany" value={form.trainCompany} onChange={handleForm} placeholder="ADY" />
              </div>
              <div className="ct-field">
                <label>Qatar №</label>
                <input name="trainNumber" value={form.trainNumber} onChange={handleForm} placeholder="T-100" />
              </div>
              <div className="ct-field">
                <label>Tarix</label>
                <input name="dueDate" type="datetime-local" value={form.dueDate} onChange={handleForm} />
              </div>
              <div className="ct-field">
                <label>Vaqon №</label>
                <input name="vagonNumber" type="number" value={form.vagonNumber} onChange={handleForm} />
              </div>
            </div>
          </section>

          {/* Marşrut Seçimi - BURADA LOCATIONS LİSTƏLƏNİR */}
          <section className="ct-section">
            <h2 className="ct-section-title"><span>📍</span> Marşrut Seçimi</h2>
            <div className="ct-grid-2">
              <div className="ct-field">
                <label>Haradan</label>
                <select name="fromId" value={form.fromId} onChange={handleForm}>
              ...
              <div className="ct-field">
                <label>Haraya</label>
                <select name="toId" value={form.toId} onChange={handleForm}>
                  <option value="">Şəhər seçin...</option>
                  {locations.map(l => (
                    <option key={l.id} value={l.id}>{l.name}</option>
                  ))}
          ...
          {/* Oturacaq Sinifləri - BURADA VARIANTS LİSTƏLƏNİR */}
          <section className="ct-section">
            ...
            {seatGroups.map((g, idx) => (
              <div key={idx} className="ct-group-box">
                <div className="ct-group-header">
                  ...
                  {seatGroups.length > 1 && (
                    <button type="button" className="ct-remove-btn" onClick={() => removeGroup(idx)}>Sil</button>
                  )}
                </div>
                <div className="ct-grid-3">
                  ...
                  <div className="ct-field">
                    ...
                    <input type="number" min={1} max={11} value={g.seatsPerRow} onChange={e => handleGroup(idx, "seatsPerRow", e.target.value)} />
                  </div>
                </div>
              </div>
            ))}
          ...
          <button type="submit" className="ct-submit-btn" disabled={loading}>
            {loading ? "Gözləyin..." : `🚆 BİLETLƏRİ YARAT (${totalSeats})`}
          </button>
        </form>
      </div>
    </div>

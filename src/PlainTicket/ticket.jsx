...
function formatTime(dateStr) {
  ...
  if (!isNaN(d.getTime())) {
    ...
    // Saat 00:00 deyilsə göstər (bəzən yalnız tarix göndərilir)
...
function FlightCard({ f, fromLabel, toLabel, onSelect, index }) {
    ...
    (async () => {
      ...
      try {
        const res  = await fetch(
          `${API_BASE}/Seat/by-ticket?TicketId=${f.id}&TicketType=plane`,
          { headers: getHeaders() }
        );
        ...
        const def = list.find((v) => v.id === f.variantId) ?? list[0] ?? null;
        setSelected(def);
      } catch (_) {
        setVariants([]);
      } finally {
  ...
  const departTime  = formatTime(f.dueDate);
  const arrivalTime = formatTime(f.arrivalDate ?? null) ?? departTime;
      ...
      <div className="ft-card-body">
        <div className="ft-top">
          <div className="ft-airline-block">
            <div className="ft-airline-logo">
              <span className="ft-airline-initial">{(f.airline || "A")[0].toUpperCase()}</span>
            </div>
          ...
          <div className="ft-flight-meta">
            ...
            <span className="ft-flight-num">{f.gate || "—"}</span>
          </div>
        </div>
        ...
        <div className="ft-route">
          <div className="ft-time-block">
            ...
            <span className="ft-city">{f.from || fromLabel}</span>
          </div>
          ...
          <div className="ft-time-block ft-time-block--right">
            ...
            <span className="ft-city ft-city--arrival">{f.to || toLabel}</span>
          </div>
        ...
        <div className="ft-bottom">
          ...
          <div className="ft-price-block">
            ...
            <button className="ft-book-btn"
              onClick={() => onSelect({ flight: f, variant: selectedVariant, fromLabel, toLabel })}>
              Book →
            </button>
          </div>
        </div>
...
export default function PlanetTicket() {
  ...
  async function search(overrideFrom, overrideTo, overrideDate) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/PlaneTicket?${params}`, { headers: getHeaders() });
  ...
  if (selectedFlight) {
    return (
      <FlightBooking
        ...
        variant={selectedFlight.variant}
        ...
        <div className="fs-card">
          <div className="fs-route-row">
            <LocationSelect locations={locations} value={fromId}
              onChange={(v) => { setFromId(v); setFlights(null); }}
            ...
            <LocationSelect locations={locations} value={toId}
              onChange={(v) => { setToId(v); setFlights(null); }}
        ...
        {flights && (
          <div className="fs-results">
            ...
            {flights.list.length === 0
              ? <div className="fs-empty"><span className="fs-empty-icon">✈</span><p>No flights found.</p></div>
